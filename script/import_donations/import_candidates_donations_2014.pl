...
sub process_uf {
    my $uf = shift;
    my $filepath = "$csvs_basedir/receitas_candidatos_2014_$uf.txt";
    print "$filepath\n";
...
sub process_line {
    ...
    my %donation_hash = WikiPoliticos::Util::make_donation_hash
      ($line, $schema->resultset('CandidateDonation2014'));

    $schema->resultset('CandidateDonation2014')->create(\%donation_hash);
}
