...
Temos que escrever um script que leia o csv vindo do Fibra e popule na
tabela locations as colunas ibge_*. Sugestão: iterar sobre linhas do
csv. Tokenizar o nome do Estado ou Município.  Procurar na tabela
locations uma linha que tenha a coluna 'token' igual ao token
gerado. Se o token gerado for encontrado na tabela locations, pegar o
código de Estado e o código de município da linha do csv que gerou o
token e colocar esses códigos na linha encontrada na tabela locations.
** finbra_despesas_*
*** escrever sql de finbra_despesas_funcoes e finbra_despesas_subfuncoes
*** rodar esses sqls para criar tabelas
*** rodar dbicdump
*** exportar tabela (qual?) do mdb para csv
